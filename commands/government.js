...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('treasury')
                .setDescription('Shows the current royal treasury balance.'))
        .addSubcommand(subcommand =>
            subcommand.setName('view_user')
    ...
    async execute(interaction) {
        ...
        const hasRequiredRole = member && member.roles.cache.some(role => ALLOWED_ROLE_IDS.includes(role.id));
            ...
            return interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription('You do not have the required role or permission to access this command.')
        ...
        const createEmbed = (title, description, color = '#00FF00', footer = interaction.client.user.tag) => {
            return new EmbedBuilder()
                .setColor(color)
                .setTitle(title)
                .setDescription(description)
                .setFooter({ text: "The treasury funds must be managed responsibly", iconURL: interaction.client.user.displayAvatarURL() });
        };
        ...
        if (subcommand === 'treasury') {
            ...
            if (!treasury) {
                return interaction.reply({
                    embeds: [createEmbed('❌ Treasury Not Found', 'The treasury user document does not exist in the database.', '#FF0000')],
                    ephemeral: true
                });
            }
            ...
            return interaction.reply({
                embeds: [createEmbed('🏛️ Royal Treasury', `The current royal treasury balance is **${treasury.gold.toLocaleString()}** gold.`)]
            });
        }
        ...
        if (subcommand === 'view_user') {
            ...
            let canClaimSalary = user.lastSalary && now - lastSalaryTimestamp >= oneWeek;
            if (canClaimSalary == null) {
                canClaimSalary = true;
            }
            const nextSalaryDate = canClaimSalary ? 'Can claim salary!' : `<t:${Math.floor((lastSalaryTimestamp + oneWeek) / 1000)}:F>`;
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: 'Job', value: user.job ? user.job : 'No job assigned', inline: false },
                    { name: 'Gold Balance', value: `${user.gold.toLocaleString()}`, inline: false },
                    { name: 'Last Salary', value: user.lastSalary ? `<t:${Math.floor(user.lastSalary / 1000)}:F>` : 'Never received salary', inline: true },

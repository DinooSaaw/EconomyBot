...
const GuildSettings = require('../Models/GuildSettings'); // Your MongoDB model for storing guild settings
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'show') {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: 'Show Criminal Record', value: settings.showCriminalRecord ? '✅ Enabled' : '❌ Disabled', inline: true },
                    { name: 'Show Treasury Balance to Everyone', value: settings.showTreasuryBalance ? '✅ Enabled' : '❌ Disabled', inline: true },
                    { name: 'Trade Notifications', value: settings.tradeNotifications ? '✅ Enabled' : '❌ Disabled', inline: true }
                )
                .setFooter({ text: 'Use /settings toggle [option] to change settings.' });
            ...
            return interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(`**${option.replace(/([A-Z])/g, ' $1').trim()}** is now ${settings[option] ? '✅ Enabled' : '❌ Disabled'}.`)
                ],
                ephemeral: true
            });
        }

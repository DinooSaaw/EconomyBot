    ...
    async execute(interaction) {
        ...
        const createEmbed = (title, description, color = '#00FF00') => {
            return new EmbedBuilder()
                ...
                .setFooter({ text: "All trades should be fair and mutually agreed upon.", iconURL: interaction.client.user.displayAvatarURL() });
        };

        // Check if the sender exists in the database
        if (!senderUser) {
            return interaction.reply({
                embeds: [createEmbed('❌ Sender Not Found', 'You do not have a profile in the economy system!', '#FF0000')],
                ephemeral: true
            });
        }
        ...
        if (!receiverUser) {
            return interaction.reply({
                embeds: [createEmbed('❌ Receiver Not Found', 'The target user does not have a profile in the economy system!', '#FF0000')],
                ephemeral: true
            });
        }
        ...
        if (senderUser.gold < amount) {
            return interaction.reply({
                embeds: [createEmbed('❌ Insufficient Gold', `You do not have enough gold to trade. You currently have **${senderUser.gold}** gold.`, '#FF0000')],
                ephemeral: true
            });
        }
        ...
        return interaction.reply({
            embeds: [createEmbed('✅ Trade Successful', `${interaction.user.displayName} has successfully traded **${amount.toLocaleString()}** gold with **${targetUser.displayName}**.\n\nYour new balance: **${senderUser.gold.toLocaleString()}** gold.\n${targetUser.displayName}'s new balance: **${receiverUser.gold.toLocaleString()}** gold.`)]
        });
    }
};
